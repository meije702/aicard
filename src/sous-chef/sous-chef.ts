...
export function createSousChef(apiKey: string) {
  ...
  return {
    ...
    async checkKitchenReadiness(recipe: Recipe, kitchen: Kitchen): Promise<string> {
      const { missing } = checkRecipeReadiness(recipe, kitchen)
      ...
      const prompt = missing.length === 0
        ? `The kitchen is fully ready to run this recipe. Tell the user what the recipe will do in plain English, step by step. Be brief and warm.\n\n${context}`
        : `The kitchen is missing some equipment. Tell the user what is missing and what they need to do to get ready.\n\n${context}`
    ...
    async getHatOptions(recipe: Recipe | null, currentStepName: string | null): Promise<string[]> {
      const context = recipe
        ? `The user is viewing the recipe "${recipe.name}". Current step: "${currentStepName ?? 'none'}".`
