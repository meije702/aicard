...
When('I parse the equipment definition', (world: World) => {
    const parsed = parseEquipmentDefinition(world.rawText!)
    if (parsed.success) {
        world.equipmentDefinition = parsed.equipment
        world.equipmentParseErrors = []
    } else {
        world.equipmentDefinition = null
        world.equipmentParseErrors = parsed.errors
    }
})
