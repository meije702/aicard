...
Given('a recipe with Wait steps of {string} and {string}', (world: World, first: string, second: string) => {
    world.recipe = RecipeBuilder.named('Test')
        .withStep('Wait first', 'wait', { 'how long': first })
        .withStep('Wait second', 'wait', { 'how long': second })
        .build()
})
...
When('I run the recipe and during review change {string} to {string}',
    async (world: World, key: string, value: string) => {
        assertExists(world.recipe)
...
When('I run the recipe and enter {string} as {string} with journal tracking',
    async (world: World, field: string, value: string) => {
        assertExists(world.recipe)
        ...
        world.runState = await runRecipe(
            ...
            (state) => {
                for (const step of state.steps) {
                    if (step.status === 'complete' && !loggedSteps.has(step.number)) {
                        loggedSteps.add(step.number)
                        const recipeStep = world.recipe?.steps[step.number - 1]
                        const cardType = recipeStep && 'card' in recipeStep ? recipeStep.card : undefined
                        if (!cardType) continue
                        world.kitchen = appendJournalEntry(world.kitchen, {
                            timestamp: new Date().toISOString(),
                            recipe: world.recipe!.name,
                            step: step.number,
                            card: cardType,
                            type: 'executed',
                        })
            ...
            async (stepIndex, interaction) => {
                ...
                if (cardType) {
                    for (const interactionField of interaction.fields.filter(f => !f.readOnly)) {
                        const original = interactionField.defaultValue ?? ''
                        const submitted = values[interactionField.key] ?? ''
...
When('I stop the recipe after step {int} completes', async (world: World, stepNumberText: string) => {
    assertExists(world.recipe)
    const stepNumber = parseInt(stepNumberText, 10)
...
Then('the recipe should be cancelled', (world: World) => {
    assertExists(world.runState)
    assertEquals(world.runState.cancelled, true)
    assertEquals(world.runState.complete, false)
})

Then('step {int} should have status {string}', (world: World, stepNumberText: string, expectedStatus: string) => {
    assertExists(world.runState)
    const stepNumber = parseInt(stepNumberText, 10)
    const step = world.runState.steps[stepNumber - 1]
    assertExists(step)
    assertEquals(step.status, expectedStatus)
})

Then('the kitchen journal should contain {int} {string} entry for {string}',
    (world: World, countText: string, entryType: string, cardType: string) => {
        const count = parseInt(countText, 10)
        const journal = world.kitchen.journal ?? []
        const matching = journal.filter(entry => entry.type === entryType && entry.card === cardType)
        assertEquals(matching.length, count)
    }
)

Then('the latest correction should record {string} changed to {string}', (world: World, before: string, after: string) => {
    const corrections = (world.kitchen.journal ?? []).filter(entry => entry.type === 'corrected')
    const latest = corrections[corrections.length - 1]
    assertExists(latest)
    assertEquals(latest.before ?? '', before)
    assertEquals(latest.after ?? '', after)
})
