...
**Status: Future work** — first-run onboarding is a separate initiative.
...
**Status: Future work** — capability-based equipment is a future milestone.
...
**Status: Future work** — equipment `mode` field (`full`/`handoff`) exists in types but UI doesn't surface it yet.
...
**Status: Fixed** — `stripStepRefs()` in `recipe-runner.ts` replaces `{step N: field}` with `"the field"` before `describe()` is called.
...
**Status: Fixed** — `CARD_LABELS` map in `StepItem.tsx` renders human-readable names.
...
**Status: Won't fix (v1)** — tweaking applies to pending steps only. The pre-step review panel gives Maria a 5-second window to tweak before each step starts.
...
**Status: Fixed** — Stop button in `RecipeView.tsx`; Run button reappears after completion; Resume + Start Fresh for paused recipes.
...
**Status: Fixed** — not reproducible in current `listen.ts` describe method.

Doubled word in Listen result. "Picked up a new new order" — the describe template has a bug.

## Finding 9

**Status: Won't fix (v1)** — expected during development; `deno compile` produces the real distribution binary.
...
**Status: Fixed** — provider panel collapses by default after refactor; full setup only expands when explicitly editing.
...
**Status: Fixed** — paused banner in `RecipeView.tsx` clearly offers Resume vs Start Fresh with last-completed-step context.
...
**Status: Fixed** — `tweakedDescriptionsRef` mechanism in `use-recipe-execution.ts` updates descriptions immediately on tweak save.
...
**Status: Fixed** — same fix as Finding 4. `stripStepRefs()` handles this.
...
**Status: Won't fix (v1)** — intentional trade-off acknowledged in the finding itself.
...
**Status: Fixed** — `send-message.ts` uses a native `<a href="mailto:...">` link instead of `window.open()`.
...
**Status: Fixed** — warning is gated by `execution.isRunning` in `RecipeView.tsx`; hides on completion.
