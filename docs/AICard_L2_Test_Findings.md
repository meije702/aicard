...
**Status: Fixed** — `tweakedDescriptionsRef` mechanism in `use-recipe-execution.ts` updates the step description immediately on tweak save.
...
**Status: Fixed** — `clearReviewTimer()` is called in `RecipeView.tsx` when the tweak editor opens, stopping the countdown.
...
**Status: Fixed** — `CardConfig.tsx` now receives `fieldDescriptions` from the card definition's `configFields` and displays title-cased labels with description hints.
...
**Status: Fixed** — `tweakedDescriptionsRef` overrides are merged into every state update via the `onStateChange` callback in `use-recipe-execution.ts`.
